...
export type Field = { key: string; label: string; type?: 'text' | 'textarea' | 'richtext' | 'number' | 'money' | 'boolean' | 'select' | 'tags' | 'image' | 'datetime' | 'date'; required?: boolean; max?: number; min?: number; options?: string[]; reference?: string; nullable?: boolean; folder?: 'menu' | 'events' | 'testimonials'; initial?: string | number | boolean };
...
export function resourceSchema(resource: Resource) {
  ...
  for (const field of resource.fields) {
    ...
    if (field.type === 'richtext') schema = z.string().max(field.max ?? 10000).transform(descriptionHtml).pipe(z.string().max(field.max ?? 10000));
      ...
      if (field.type === 'image') schema = value.refine((input) => /^(images\/)?(menu|events|testimonials)\/[a-zA-Z0-9_-]+\.(jpg|jpeg|png|webp|avif)$/.test(input) || /^https:\/\/images\.unsplash\.com\/photo-[a-zA-Z0-9-]+(?:\?[^\s]*)?$/.test(input), 'Choose an uploaded image or supported stock photo');
    }
